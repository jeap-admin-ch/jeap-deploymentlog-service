...
- Include cancelled deployments in terminal deployment counters and duration metrics using
  `result="cancelled"`.
...
- Exclude deployments below the configured flow start stage from flow assignment instead of classifying them as
  `AD_HOC`.
...
- Add the role-protected `/api/system-groups` administration API for creating, reading, renaming and deleting system
  groups and for assigning each system to at most one group. Group names are trimmed and case-insensitively unique;
  they have no application-defined length limit. Deleting a group keeps its systems and removes their assignments.
  Errors use Problem Details with stable error codes and do not expose internal exception messages.

### Migration

The Flyway migration creates the `system_group` table with a case-insensitive name uniqueness constraint and the
nullable, indexed `system.system_group_id` foreign key. Existing systems remain ungrouped. Deleting a group sets this
foreign key to `NULL` and never deletes a system. No application configuration is required to enable the administrative
API.
...
### Added
- Add `CANCELLED` as a terminal deployment state.
...
### Dependencies
- **net.javacrumbs.shedlock:shedlock-spring**: 7.7.0 → 7.8.0 (minor)
- **net.javacrumbs.shedlock:shedlock-provider-jdbc-template**: 7.7.0 → 7.8.0 (minor)
...
### Changed
- Ready-for-deploy check: jira issues that cannot be resolved (issue does not exist, project not visible, or key not a
  valid jira issue key) now yield a `WARNING` check result with a diagnostic message instead of an unhandled exception
  (HTTP 500). `DeploymentCheckResultDto` additionally reports `issuesWithoutLabel`, `issuesNotFound` and
  `projectsNotVisible` (backward compatible).
- Jira search: issue keys are validated, normalized and quoted in the JQL query (keys like `AND-1` no longer break the
  query), searches are chunked (previously more than 50 issues were silently not checked).
- If jira is unavailable or rejects the deployment log service itself (e.g. invalid credentials), the deployment API
  returns HTTP 503 with a meaningful message instead of passing the jira status through, and logs at ERROR level.
- New optional configuration property `jeap.deploymentlog.jira.retry-delay-ms` (default 2000).
...
### Dependencies
- **net.javacrumbs.shedlock:shedlock-spring**: 4.33.0 → 7.7.0 (major)
- **net.javacrumbs.shedlock:shedlock-provider-jdbc-template**: 4.33.0 → 7.7.0 (major)
- **ch.admin.bit.jeap:jeap-spring-boot-parent**: 35.11.0 → 35.11.1 (patch)
- **org.sahli.asciidoc.confluence.publisher:asciidoc-confluence-publisher-client**: 0.33.0 → 0.34.0 (minor)
...
- Remove webflux test dependencies
