...
public class DocgenLocks {
    ...
    // Serialize lock extensions as well, so they cannot create a burst of datasource usage. An extension is a short
    // database operation and runs well within the KeepAliveLockProvider interval.
    private static final int LOCK_EXTENDER_THREADS = 1;
    // Wait at most this duration until giving up trying to acquire the lock
    @Value("${jeap.deploymentlog.documentation-generator.lock-acquire-timeout:PT30S}")
    private Duration tryAcquireTimeout = Duration.ofSeconds(30);

    private final LockProvider lockProvider;
    private final ScheduledExecutorService lockExtender;

    public DocgenLocks(LockProvider lockProvider) {
    ...
    void runIfLockAquiredBeforeTimeout(String systemName, Runnable task) {
        ...
        tryAcquireLockWithTimeout(lockName).ifPresentOrElse(
                ...
                () -> log.info("Lock {} is busy; deferring page generation to the repair job", lockName));
    ...
    public <T> Optional<T> tryRunWithDocumentationStructureLock(Supplier<T> task) {
        Optional<SimpleLock> lock = tryAcquireLockWithTimeout(DOCUMENTATION_STRUCTURE_LOCK_NAME);
        if (lock.isEmpty()) {
            log.info("Documentation structure is busy; deferring page generation to the repair job");
            return Optional.empty();
        }
        return Optional.ofNullable(runLockedTask(task, DOCUMENTATION_STRUCTURE_LOCK_NAME, lock.get()));
    }

    private <T> T runLockedTask(Supplier<T> task, String lockName, SimpleLock lock) {
        try {
    ...
    private Optional<SimpleLock> tryAcquireLockWithTimeout(String lockName) {
        ...
        for (int i = 1; ; i++) {
            ...
            log.debug("Docgen lock {} is busy (retry #{})", lockName, i);
